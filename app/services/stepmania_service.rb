...
class StepmaniaService
  STEPMANIA_URL = Rails.configuration.x.gign['stepmania']['json_url']
  ...
  def self.update_stepmania_bdd!
    client_stepmania = Mysql2::Client.new(
      host: Rails.configuration.x.gign['stepmania']['host'],
      username: Rails.configuration.x.gign['stepmania']['user'],
      password: Rails.configuration.x.gign['stepmania']['password'],
      database: Rails.configuration.x.gign['stepmania']['database'])
  ...
  def self.update_stepmania_song!
    OpenSmoSong.all.each do |song|
      song.update_columns(subtitle: '') if song.subtitle.nil?
